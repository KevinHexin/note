...
#### D. 优化使用锁页内存的方法
如果待初始化的内存每次初始化的时候大小都是固定的, 那么应该采用整个工程代码初始化的时候申请锁页内存或者注册锁页内存的方法, 在整个工程代码结束的时候释放内存, 这样就避免了频繁的调用或释放.
另外注意, cudaSetDeviceFlags(cudaDeviceMapHost)函数只能在初始化的时候调用一次, 不能频繁调用
